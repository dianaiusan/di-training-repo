"""Shared inline Lucide SVG helpers for generator scripts."""
...
def render_lucide_icon(icon_name: str, fallback_icon: str = "tag", size_class: str = "") -> str:
    icon_key = str(icon_name or fallback_icon)
    svg_paths = LUCIDE_PATHS.get(icon_key, LUCIDE_PATHS[fallback_icon])
    classes = f"lucide lucide-{icon_key}"
    if size_class:
        classes = f"{classes} {size_class}"
    return (
