...
### 5. Optional: run spell check (British English)
...
```bash
./scripts/spellcheck/run_spellcheck.sh
```

Notes:
- Uses British English (`en-GB`) for markdown content in `docs/` and `files/`.
- Project-specific accepted terms are maintained in `scripts/spellcheck/cspell-words.txt`.
- Requires Node.js (the CI workflow runs this automatically on push/PR).

## When You Must Run scripts/generate_all.py

Run it after changes to any of these:
- docs/explore/all-training/*.md
- docs/explore/**/*.md
- docs/events/**/*.md
- scripts/*.py
